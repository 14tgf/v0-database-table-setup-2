  ...
  return (
    ...
    >
      ...
      <motion.div variants={staggerItem} className="mb-4">
        <p className="text-xs font-semibold text-muted-foreground mb-2">Accepted document types:</p>
        <div className="flex flex-wrap gap-2">
          {documentTypes.map((type) => (
            <span key={type} className="px-2.5 py-1 bg-white/5 border border-white/10 rounded-lg text-xs text-muted-foreground">
              {type}
            </span>
          ))}
        </div>
      ...
      <motion.div variants={staggerItem}>
        {!uploadedFile ? (
          ...
          >
            ...
            <p className="text-xs text-muted-foreground mb-3">Supported formats: {acceptedFormats.join(', ')}</p>
