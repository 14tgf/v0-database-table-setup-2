...
export function CryptoForm({ type, onSubmit, autoAmount }: CryptoFormProps) {
  ...
  useEffect(() => {
    const fetchPaymentMethods = async () => {
      try {
        console.log('[v0] CRYPTO FORM - Fetching payment methods');
        setLoading(true);
        setError(null);
        ...
        if (!response.ok) {
          ...
          console.warn('[v0] CRYPTO FORM - Failed to fetch:', response.status, errorData);
        ...
        console.error('[v0] CRYPTO FORM - Error:', errorMsg);
