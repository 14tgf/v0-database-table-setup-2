...
export function SidebarMenu({ isOpen, onClose }: SidebarMenuProps) {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        console.error('[v0] Failed to fetch user data for sidebar:', error);
      } finally {
  ...
  const menuItems = [
    ...
    { icon: TrendingUp, label: 'Investment Plans', href: '/dashboard/investment-plans' },
  ...
  const userInitial = userData.fullName ? userData.fullName[0].toUpperCase() : 'U';
